...
void* imgui_Spoof_memcpyf_(void* dest, const void* src, unsigned __int64 count)
{
...
int BetterstrlenA(const char* string)
{
	int cnt = 0;
	if (string)
	{
...
const char* strcpyf_A(char* buffer, const char* string) {
	char* ret = buffer;
...
void ImGui::StyleColorsDark(ImGuiStyle* dst)
{
	...
	colors[ImGuiCol_CheckMark] = ImColor(3, 169, 252, 255);
	colors[ImGuiCol_SliderGrab] = ImColor(3, 169, 252, 255);
	colors[ImGuiCol_SliderGrabActive] = ImColor(3, 169, 252, 255);
	colors[ImGuiCol_Button] = ImColor(40, 40, 40, 255);
	colors[ImGuiCol_ButtonHovered] = ImColor(3, 169, 252, 255);
	colors[ImGuiCol_ButtonActive] = ImColor(3, 169, 252, 255);
...
void ImDrawList::ChannelsMerge()
{
	...
	{
		ImDrawChannel& ch = _Channels[i];
		if (int sz = ch.CmdBuffer.Size) { imgui_Spoof_memcpyf_(cmd_write, ch.CmdBuffer.Data, sz * sizeof(ImDrawCmd)); cmd_write += sz; }
		if (int sz = ch.IdxBuffer.Size) { imgui_Spoof_memcpyf_(_IdxWritePtr, ch.IdxBuffer.Data, sz * sizeof(ImDrawIdx)); _IdxWritePtr += sz; }
	}
...
void ImDrawList::ChannelsSetCurrent(int idx)
{
	...
	imgui_Spoof_memcpyf_(&_Channels.Data[_ChannelsCurrent].CmdBuffer, &CmdBuffer, sizeof(CmdBuffer)); // copy 12 bytes, four times
	imgui_Spoof_memcpyf_(&_Channels.Data[_ChannelsCurrent].IdxBuffer, &IdxBuffer, sizeof(IdxBuffer));
	_ChannelsCurrent = idx;
	imgui_Spoof_memcpyf_(&CmdBuffer, &_Channels.Data[_ChannelsCurrent].CmdBuffer, sizeof(CmdBuffer));
	imgui_Spoof_memcpyf_(&IdxBuffer, &_Channels.Data[_ChannelsCurrent].IdxBuffer, sizeof(IdxBuffer));
...
void ImDrawList::AddText(const ImFont* font, float font_size, const ImVec2& pos, ImU32 col, const char* text_begin, const char* text_end, float wrap_width, const ImVec4* cpu_fine_clip_rect)
{
	...
	if (text_end == NULL)
		text_end = text_begin + BetterstrlenA(text_begin);
...
ImFont* ImFontAtlas::AddFont(const ImFontConfig* font_cfg)
{
	...
	if (!new_font_cfg.FontDataOwnedByAtlas)
	{
		...
		imgui_Spoof_memcpyf_(new_font_cfg.FontData, font_cfg->FontData, (size_t)new_font_cfg.FontDataSize);
	}
...
ImFont* ImFontAtlas::AddFontDefault(const ImFontConfig* font_cfg_template)
{
	...
	if (font_cfg.Name[0] == '\0') strcpyf_A(font_cfg.Name, skCrypt("ProggyClean.ttf, 13px"));
...
{
	int data_size = 0;
	void* data = ImFileLoadToMemory(filename, skCrypt("rb"), &data_size, 0);
	if (!data)
	{
	...
	if (font_cfg.Name[0] == '\0')
	{
		...
		const char* p;
		for (p = filename + BetterstrlenA(filename); p > filename && p[-1] != '/' && p[-1] != '\\'; p--) {}
		snprintf(font_cfg.Name, IM_ARRAYSIZE(font_cfg.Name), skCrypt("%s, %.0fpx"), p, size_pixels);
	}
...
{
	int compressed_ttf_size = (((int)BetterstrlenA(compressed_ttf_data_base85) + 4) / 5) * 4;
...
const ImWchar*  ImFontAtlas::GetGlyphRangesJapanese()
{
	...
	if (!full_ranges_unpacked)
	{
		...
		imgui_Spoof_memcpyf_(full_ranges, base_ranges, sizeof(base_ranges));
...
{
	if (!text_end)
		text_end = text_begin + BetterstrlenA(text_begin); // FIXME-OPT: Need to avoid this.
...
{
	if (!text_end)
		text_end = text_begin + BetterstrlenA(text_begin); // ImGui functions generally already provides a valid text_end, so this is merely to handle direct calls.
...
static void stb__lit(unsigned char *data, unsigned int length)
{
	...
	imgui_Spoof_memcpyf_(stb__dout, data, length);
	stb__dout += length;
}
