...
namespace SpoofWinFuncs {
	...
	void* memcpyf_(void* dest, const void* src, unsigned __int64 count)
	{
...
{
	size_t len = SpoofWinFuncs::strlenf_(str) + 1;
	void* buf = ImGui::MemAlloc(len);
	return (char*)SpoofWinFuncs::memcpyf_(buf, (const void*)str, len);
}
...
{
	if (!needle_end)
		needle_end = needle + SpoofWinFuncs::strlenf_(needle);
...
{
	Filters.resize(0);
	TextRange input_range(InputBuf, InputBuf + SpoofWinFuncs::strlenf_(InputBuf));
...
void ImGui::NewFrame()
{
	...
	IM_ASSERT(g.IO.DeltaTime >= 0.0f && skCrypt("Need a positive DeltaTime (zero is tolerated but will cause some timing issues)"));
	IM_ASSERT(g.IO.DisplaySize.x >= 0.0f && g.IO.DisplaySize.y >= 0.0f && skCrypt("Invalid DisplaySize value"));
	IM_ASSERT(g.IO.Fonts->Fonts.Size > 0 && skCrypt("Font Atlas not created. Did you call io.Fonts->GetTexDataAsRGBA32 / GetTexDataAsAlpha8 ?"));
	IM_ASSERT(g.IO.Fonts->Fonts[0]->IsLoaded() && skCrypt("Font Atlas not created. Did you call io.Fonts->GetTexDataAsRGBA32 / GetTexDataAsAlpha8 ?"));
	IM_ASSERT(g.Style.CurveTessellationTol > 0.0f && skCrypt("Invalid style setting"));
	IM_ASSERT(g.Style.Alpha >= 0.0f && g.Style.Alpha <= 1.0f && skCrypt("Invalid style setting. Alpha cannot be negative (allows us to avoid a few clamps in color computations)"));
	IM_ASSERT((g.FrameCount == 0 || g.FrameCountEnded == g.FrameCount) && skCrypt("Forgot to call Render() or EndFrame() at the end of the previous frame?"));
	for (int n = 0; n < ImGuiKey_COUNT; n++)
		IM_ASSERT(g.IO.KeyMap[n] >= -1 && g.IO.KeyMap[n] < IM_ARRAYSIZE(g.IO.KeysDown) && skCrypt("io.KeyMap[] contains an out of bound value (need to be 0..512, or -1 for unmapped key)"));
	...
	SpoofWinFuncs::memcpyf_(g.IO.KeysDownDurationPrev, g.IO.KeysDownDuration, sizeof(g.IO.KeysDownDuration));
...
static void SettingsHandlerWindow_ReadLine(ImGuiContext*, ImGuiSettingsHandler*, void* entry, const char* line)
{
	...
	float x, y;
	int i;
	if (sscanf(line, skCrypt("Pos=%f,%f"), &x, &y) == 2)         settings->Pos = ImVec2(x, y);
	else if (sscanf(line, skCrypt("Size=%f,%f"), &x, &y) == 2)   settings->Size = ImMax(ImVec2(x, y), GImGui->Style.WindowMinSize);
	else if (sscanf(line, skCrypt("Collapsed=%d"), &i) == 1)     settings->Collapsed = (i != 0);
}
...
static void SettingsHandlerWindow_WriteAll(ImGuiContext* imgui_ctx, ImGuiSettingsHandler* handler, ImGuiTextBuffer* buf)
{
	...
	for (int i = 0; i != g.SettingsWindows.Size; i++)
	{
		...
		if (const char* p = strstr(name, skCrypt("###")))  // Skip to the "###" marker if any. We don't skip past to match the behavior of GetID()
			name = p;
		buf->appendf(skCrypt("[%s][%s]\n"), handler->TypeName, name);
		buf->appendf(skCrypt("Pos=%d,%d\n"), (int)settings->Pos.x, (int)settings->Pos.y);
		buf->appendf(skCrypt("Size=%d,%d\n"), (int)settings->Size.x, (int)settings->Size.y);
		buf->appendf(skCrypt("Collapsed=%d\n"), settings->Collapsed);
		buf->appendf(skCrypt("\n"));
	}
}
...
void ImGui::Initialize()
{
	...
	ini_handler.TypeName = skCrypt("Window");
	ini_handler.TypeHash = ImHash(skCrypt("Window"), 0, 0);
...
static void LoadIniSettingsFromDisk(const char* ini_filename)
{
	if (!ini_filename)
		return;
	char* file_data = (char*)ImFileLoadToMemory(ini_filename, skCrypt("rb"), NULL, +1);
	if (!file_data)
		return;
...
static void LoadIniSettingsFromMemory(const char* buf_readonly)
{
	...
	char* buf_end = buf + SpoofWinFuncs::strlenf_(buf);
...
static void SaveIniSettingsToDisk(const char* ini_filename)
{
	...
	FILE* f = ImFileOpen(ini_filename, skCrypt("wt"));
	if (!f)
		return;
...
static void AddDrawListToDrawData(ImVector<ImDrawList*>* out_render_list, ImDrawList* draw_list)
{
	...
	if (sizeof(ImDrawIdx) == 2)
		IM_ASSERT(draw_list->_VtxCurrentIdx < (1 << 16) && skCrypt("Too many vertices in ImDrawList using 16-bit indices. Read comment above"));
...
void ImDrawDataBuilder::FlattenIntoSingleLayer()
{
	...
	for (int layer_n = 1; layer_n < IM_ARRAYSIZE(Layers); layer_n++)
	{
		...
		if (layer.empty())
			continue;
		SpoofWinFuncs::memcpyf_(&Layers[0][n], &layer[0], layer.Size * sizeof(ImDrawList*));
		n += layer.Size;
		layer.resize(0);
	}
}
...
void ImGui::Render()
{
	...
	if (g.Style.Alpha > 0.0f)
	{
		...
		if (g.IO.MouseDrawCursor)
		{
			...
			g.OverlayDrawList.AddImage(tex_id, pos + ImVec2(1, 0), pos + ImVec2(1, 0) + size, cursor_data.TexUvMin[1], cursor_data.TexUvMax[1], IM_COL32(0, 0, 0, 255));        // Shadow
			g.OverlayDrawList.AddImage(tex_id, pos + ImVec2(2, 0), pos + ImVec2(2, 0) + size, cursor_data.TexUvMin[1], cursor_data.TexUvMax[1], IM_COL32(0, 0, 0, 255));        // Shadow
			g.OverlayDrawList.AddImage(tex_id, pos, pos + size, cursor_data.TexUvMin[1], cursor_data.TexUvMax[1], IM_COL32(255, 130, 0, 255));       // Black border
			g.OverlayDrawList.AddImage(tex_id, pos, pos + size, cursor_data.TexUvMin[0], cursor_data.TexUvMax[0], IM_COL32(255, 130, 0, 255)); // White fill
	...
	for (;;)
	{
		...
		if (line_end != NULL && !(is_last_line && (line_end - text_remaining) == 0))
		{
			...
			//if (log_new_line || !is_first_line)
				//ImGui::LogText(IM_NEWLINE "%*s%.*s", tree_depth * 4, skCrypt(""), char_count, text_remaining);
			//else
				//ImGui::LogText(skCrypt(" %.*s"), char_count, text_remaining);
		}
...
void ImGui::RenderText(ImVec2 pos, const char* text, const char* text_end, bool hide_text_after_hash)
{
	...
	{
		if (!text_end)
			text_end = text + SpoofWinFuncs::strlenf_(text); // FIXME-OPT
...
void ImGui::BeginTooltipEx(ImGuiWindowFlags extra_flags, bool override_previous_tooltip)
{
	ImGuiContext& g = *GImGui;
	char window_name[16];
	ImFormatString(window_name, IM_ARRAYSIZE(window_name), skCrypt("##Tooltip_%02d"), g.TooltipOverrideCount);
...
bool ImGui::BeginPopupEx(ImGuiID id, ImGuiWindowFlags extra_flags)
{
	...
	if (extra_flags & ImGuiWindowFlags_ChildMenu)
		ImFormatString(name, IM_ARRAYSIZE(name), skCrypt("##Menu_%02d"), g.CurrentPopupStack.Size);    // Recycle windows based on depth
	else
		ImFormatString(name, IM_ARRAYSIZE(name), skCrypt("##Popup_%08x"), id); // Not recycling, so we can close/open during the same frame
...
static bool BeginChildEx(const char* name, ImGuiID id, const ImVec2& size_arg, bool border, ImGuiWindowFlags extra_flags)
{
	...
	char title[256];
	if (name)
		ImFormatString(title, IM_ARRAYSIZE(title), skCrypt("%s/%s_%08X"), parent_window->Name, name, id);
	else
		ImFormatString(title, IM_ARRAYSIZE(title), skCrypt("%s/%08X"), parent_window->Name, id);
...
static void CheckStacksSize(ImGuiWindow* window, bool write)
{
	...
	{ int current = window->IDStack.Size;       if (write) *p_backup = current; else IM_ASSERT(*p_backup == current && skCrypt("PushID/PopID or TreeNode/TreePop Mismatch!"));   p_backup++; }    // Too few or too many PopID()/TreePop()
	{ int current = window->DC.GroupStack.Size; if (write) *p_backup = current; else IM_ASSERT(*p_backup == current && skCrypt("BeginGroup/EndGroup Mismatch!"));                p_backup++; }    // Too few or too many EndGroup()
	{ int current = g.CurrentPopupStack.Size;   if (write) *p_backup = current; else IM_ASSERT(*p_backup == current && skCrypt("BeginMenu/EndMenu or BeginPopup/EndPopup Mismatch")); p_backup++; }// Too few or too many EndMenu()/EndPopup()
	{ int current = g.ColorModifiers.Size;      if (write) *p_backup = current; else IM_ASSERT(*p_backup == current && skCrypt("PushStyleColor/PopStyleColor Mismatch!"));       p_backup++; }    // Too few or too many PopStyleColor()
	{ int current = g.StyleModifiers.Size;      if (write) *p_backup = current; else IM_ASSERT(*p_backup == current && skCrypt("PushStyleVar/PopStyleVar Mismatch!"));           p_backup++; }    // Too few or too many PopStyleVar()
	{ int current = g.FontStack.Size;           if (write) *p_backup = current; else IM_ASSERT(*p_backup == current && skCrypt("PushFont/PopFont Mismatch!"));                   p_backup++; }    // Too few or too many PopFont()
	...
	if (first_begin_of_the_frame)
	{
		...
		if (!(flags & ImGuiWindowFlags_NoTitleBar))
		{
			...
			if (p_open != NULL)
			{
				...
				if (CloseButton(window->GetID(skCrypt("#CLOSE")), window->Rect().GetTR() + ImVec2(-PAD - rad, PAD + rad), rad))
					*p_open = false;
			}
...
void ImGui::Scrollbar(ImGuiLayoutType direction)
{
	...
	const ImGuiStyle& style = g.Style;
	const ImGuiID id = window->GetID(horizontal ? skCrypt("#SCROLLX") : skCrypt("#SCROLLY"));
...
const char* ImGui::GetStyleColorName(ImGuiCol idx)
{
	...
	switch (idx)
	{
	case ImGuiCol_Text: return skCrypt("Text");
	case ImGuiCol_TextDisabled: return skCrypt("TextDisabled");
	case ImGuiCol_WindowBg: return skCrypt("WindowBg");
	case ImGuiCol_ChildBg: return skCrypt("ChildBg");
	case ImGuiCol_PopupBg: return skCrypt("PopupBg");
	case ImGuiCol_Border: return skCrypt("Border");
	case ImGuiCol_BorderShadow: return skCrypt("BorderShadow");
	case ImGuiCol_FrameBg: return skCrypt("FrameBg");
	case ImGuiCol_FrameBgHovered: return skCrypt("FrameBgHovered");
	case ImGuiCol_FrameBgActive: return skCrypt("FrameBgActive");
	case ImGuiCol_TitleBg: return skCrypt("TitleBg");
	case ImGuiCol_TitleBgActive: return skCrypt("TitleBgActive");
	case ImGuiCol_TitleBgCollapsed: return skCrypt("TitleBgCollapsed");
	case ImGuiCol_MenuBarBg: return skCrypt("MenuBarBg");
	case ImGuiCol_ScrollbarBg: return skCrypt("ScrollbarBg");
	case ImGuiCol_ScrollbarGrab: return skCrypt("ScrollbarGrab");
	case ImGuiCol_ScrollbarGrabHovered: return skCrypt("ScrollbarGrabHovered");
	case ImGuiCol_ScrollbarGrabActive: return skCrypt("ScrollbarGrabActive");
	case ImGuiCol_CheckMark: return skCrypt("CheckMark");
	case ImGuiCol_SliderGrab: return skCrypt("SliderGrab");
	case ImGuiCol_SliderGrabActive: return skCrypt("SliderGrabActive");
	case ImGuiCol_Button: return skCrypt("Button");
	case ImGuiCol_ButtonHovered: return skCrypt("ButtonHovered");
	case ImGuiCol_ButtonActive: return skCrypt("ButtonActive");
	case ImGuiCol_Header: return skCrypt("Header");
	case ImGuiCol_HeaderHovered: return skCrypt("HeaderHovered");
	case ImGuiCol_HeaderActive: return skCrypt("HeaderActive");
	case ImGuiCol_Separator: return skCrypt("Separator");
	case ImGuiCol_SeparatorHovered: return skCrypt("SeparatorHovered");
	case ImGuiCol_SeparatorActive: return skCrypt("SeparatorActive");
	case ImGuiCol_ResizeGrip: return skCrypt("ResizeGrip");
	case ImGuiCol_ResizeGripHovered: return skCrypt("ResizeGripHovered");
	case ImGuiCol_ResizeGripActive: return skCrypt("ResizeGripActive");
	case ImGuiCol_CloseButton: return skCrypt("CloseButton");
	case ImGuiCol_CloseButtonHovered: return skCrypt("CloseButtonHovered");
	case ImGuiCol_CloseButtonActive: return skCrypt("CloseButtonActive");
	case ImGuiCol_PlotLines: return skCrypt("PlotLines");
	case ImGuiCol_PlotLinesHovered: return skCrypt("PlotLinesHovered");
	case ImGuiCol_PlotHistogram: return skCrypt("PlotHistogram");
	case ImGuiCol_PlotHistogramHovered: return skCrypt("PlotHistogramHovered");
	case ImGuiCol_TextSelectedBg: return skCrypt("TextSelectedBg");
	case ImGuiCol_ModalWindowDarkening: return skCrypt("ModalWindowDarkening");
	case ImGuiCol_DragDropTarget: return skCrypt("DragDropTarget");
	}
	IM_ASSERT(0);
	return skCrypt("Unknown");
}
...
bool ImGui::ImageButton(ImTextureID user_texture_id, const ImVec2& size, const ImVec2& uv0, const ImVec2& uv1, int frame_padding, const ImVec4& bg_col, const ImVec4& tint_col)
{
	...
	const ImGuiID id = window->GetID(skCrypt("#image"));
	PopID();
...
void ImGui::LogToFile(int max_depth, const char* filename)
{
	...
	g.LogFile = ImFileOpen(filename, skCrypt("ab"));
	if (!g.LogFile)
	{
...
void ImGui::LogButtons()
{
	...
	PushID(skCrypt("LogButtons"));
	const bool log_to_tty = Button(skCrypt("Log To TTY")); SameLine();
	const bool log_to_file = Button(skCrypt("Log To File")); SameLine();
	const bool log_to_clipboard = Button(skCrypt("Log To Clipboard")); SameLine();
	PushItemWidth(80.0f);
	PushAllowKeyboardFocus(false);
	SliderInt(skCrypt("Depth"), &g.LogAutoExpandMaxDepth, 0, 9, NULL);
...
bool ImGui::TreeNodeBehavior(ImGuiID id, ImGuiTreeNodeFlags flags, const char* label, const char* label_end)
{
	...
	if (display_frame)
	{
		...
		if (g.LogEnabled)
		{
			...
			//const char log_prefix[] = (const char*)skCrypt("\n##");
	...
	else
	{
		...
		if (g.LogEnabled)
			LogRenderedText(&text_pos, skCrypt(">"));
...
static inline void DataTypeFormatString(ImGuiDataType data_type, void* data_ptr, int decimal_precision, char* buf, int buf_size)
{
	if (data_type == ImGuiDataType_Int)
	{
		if (decimal_precision < 0)
			ImFormatString(buf, buf_size, skCrypt("%d"), *(int*)data_ptr);
		else
			ImFormatString(buf, buf_size, skCrypt("%.*d"), decimal_precision, *(int*)data_ptr);
	}
	else if (data_type == ImGuiDataType_Float)
	{
		if (decimal_precision < 0)
			ImFormatString(buf, buf_size, skCrypt("%f"), *(float*)data_ptr);     // Ideally we'd have a minimum decimal precision of 1 to visually denote that it is a float, while hiding non-significant digits?
		else
			ImFormatString(buf, buf_size, skCrypt("%.*f"), decimal_precision, *(float*)data_ptr);
	}
}
...
static bool DataTypeApplyOpFromText(const char* buf, const char* initial_value_buf, ImGuiDataType data_type, void* data_ptr, const char* scalar_format)
{
	...
	if (data_type == ImGuiDataType_Int)
	{
		...
		float arg1f = 0.0f;
		if (op == '+') { if (sscanf(buf, skCrypt("%f"), &arg1f) == 1) *v = (int)(arg0i + arg1f); }                 // Add (use "+-" to subtract)
		else if (op == '*') { if (sscanf(buf, skCrypt("%f"), &arg1f) == 1) *v = (int)(arg0i * arg1f); }                 // Multiply
		else if (op == '/') { if (sscanf(buf, skCrypt("%f"), &arg1f) == 1 && arg1f != 0.0f) *v = (int)(arg0i / arg1f); }// Divide
...
bool ImGui::SliderBehavior(const ImRect& frame_bb, ImGuiID id, float* v, float v_min, float v_max, float power, int decimal_precision, ImGuiSliderFlags flags)
{
	...
	window->DrawList->AddCircleFilled(ImVec2(grab_bb.Min.x + 6, grab_bb.Min.y + ((grab_bb.Max.y - grab_bb.Min.y) / 2)), 5, ImColor(3, 169, 252, 255));
	window->DrawList->AddLine(ImVec2(frame_bb.Min.x, frame_bb.Min.y + ((frame_bb.Max.y - frame_bb.Min.y) / 2)), ImVec2(frame_bb.Min.x + 190, frame_bb.Min.y + ((frame_bb.Max.y - frame_bb.Min.y) / 2)), ImColor(3, 169, 252, 255));
	window->DrawList->AddLine(ImVec2(frame_bb.Min.x, frame_bb.Min.y + ((frame_bb.Max.y - frame_bb.Min.y) / 2)), ImVec2(grab_bb.Min.x, frame_bb.Min.y + ((frame_bb.Max.y - frame_bb.Min.y) / 2)), ImColor(3, 169, 252, 255));
...
//   "Gold: %.0f"   Gold: 1
bool ImGui::SliderFloat(const char* label, float* v, float v_min, float v_max, const char* display_format, float power, float textxpos)
{
	ImGuiWindow* window = GetCurrentWindow();
	if (window->SkipItems)
		return false;
	...
	//RenderTextClipped(frame_bb.Min, frame_bb.Max, value_buf, value_buf_end, NULL, ImVec2(textxpos, 0.00f));
...
bool ImGui::SliderAngle(const char* label, float* v_rad, float v_degrees_min, float v_degrees_max)
{
	...
	bool value_changed = SliderFloat(label, &v_deg, v_degrees_min, v_degrees_max, skCrypt("%.0f deg"), 1.0f);
...
bool ImGui::SliderFloatN(const char* label, float* v, int components, float v_min, float v_max, const char* display_format, float power)
{
	...
	{
		PushID(i);
		value_changed |= SliderFloat(skCrypt("##v"), &v[i], v_min, v_max, display_format, power);
...
bool ImGui::SliderIntN(const char* label, int* v, int components, int v_min, int v_max, const char* display_format)
{
	...
	{
		PushID(i);
		value_changed |= SliderInt(skCrypt("##v"), &v[i], v_min, v_max, display_format);
		SameLine(0, g.Style.ItemInnerSpacing.x);
		PopID();
		PopItemWidth();
	}
...
bool ImGui::DragFloatN(const char* label, float* v, int components, float v_speed, float v_min, float v_max, const char* display_format, float power)
{
	...
	{
		PushID(i);
		value_changed |= DragFloat(skCrypt("##v"), &v[i], v_speed, v_min, v_max, display_format, power);
		SameLine(0, g.Style.ItemInnerSpacing.x);
		PopID();
		PopItemWidth();
	}
...
bool ImGui::DragFloatRange2(const char* label, float* v_current_min, float* v_current_max, float v_speed, float v_min, float v_max, const char* display_format, const char* display_format_max, float power)
{
	...
	bool value_changed = DragFloat(skCrypt("##min"), v_current_min, v_speed, (v_min >= v_max) ? -FLT_MAX : v_min, (v_min >= v_max) ? *v_current_max : ImMin(v_max, *v_current_max), display_format, power);
	PopItemWidth();
	SameLine(0, g.Style.ItemInnerSpacing.x);
	value_changed |= DragFloat(skCrypt("##max"), v_current_max, v_speed, (v_min >= v_max) ? *v_current_min : ImMax(v_min, *v_current_min), (v_min >= v_max) ? FLT_MAX : v_max, display_format_max ? display_format_max : display_format, power);
	PopItemWidth();
	SameLine(0, g.Style.ItemInnerSpacing.x);
...
bool ImGui::DragIntN(const char* label, int* v, int components, float v_speed, int v_min, int v_max, const char* display_format)
{
	...
	{
		PushID(i);
		value_changed |= DragInt(skCrypt("##v"), &v[i], v_speed, v_min, v_max, display_format);
		SameLine(0, g.Style.ItemInnerSpacing.x);
		PopID();
		PopItemWidth();
	}
...
bool ImGui::DragIntRange2(const char* label, int* v_current_min, int* v_current_max, float v_speed, int v_min, int v_max, const char* display_format, const char* display_format_max)
{
	...
	bool value_changed = DragInt(skCrypt("##min"), v_current_min, v_speed, (v_min >= v_max) ? INT_MIN : v_min, (v_min >= v_max) ? *v_current_max : ImMin(v_max, *v_current_max), display_format);
	PopItemWidth();
	SameLine(0, g.Style.ItemInnerSpacing.x);
	value_changed |= DragInt(skCrypt("##max"), v_current_max, v_speed, (v_min >= v_max) ? *v_current_min : ImMax(v_min, *v_current_min), (v_min >= v_max) ? INT_MAX : v_max, display_format_max ? display_format_max : display_format);
	PopItemWidth();
	SameLine(0, g.Style.ItemInnerSpacing.x);
	...
	if (values_count > 0)
	{
		...
		if (hovered)
		{
			...
			if (plot_type == ImGuiPlotType_Lines)
				SetTooltip(skCrypt("%d: %8.4g\n%d: %8.4g"), v_idx, v0, v_idx + 1, v1);
			else if (plot_type == ImGuiPlotType_Histogram)
				SetTooltip(skCrypt("%d: %8.4g"), v_idx, v0);
			v_hovered = v_idx;
		}
...
void ImGui::ProgressBar(float fraction, const ImVec2& size_arg, const char* overlay)
{
	...
	if (!overlay)
	{
		ImFormatString(overlay_buf, IM_ARRAYSIZE(overlay_buf), skCrypt("%.0f%%"), fraction * 100 + 0.01f);
		overlay = overlay_buf;
	}
...
bool ImGui::Checkbox(const char* label, bool* v)
{
	...
	if (label_size.x > 0)
		SameLine(0, style.ItemInnerSpacing.x - 1);
...
bool ImGui::RadioButton(const char* label, bool active)
{
	...
	if (g.LogEnabled)
		LogRenderedText(&text_bb.Min, active ? skCrypt("(x)") : skCrypt("( )"));
	if (label_size.x > 0.0f)
...
#define STB_TEXTEDIT_MOVEWORDRIGHT  STB_TEXTEDIT_MOVEWORDRIGHT_IMPL
	...
	static bool STB_TEXTEDIT_INSERTCHARS(STB_TEXTEDIT_STRING* obj, int pos, const ImWchar* new_text, int new_text_len)
	{
		...
		SpoofWinFuncs::memcpyf_(text + pos, new_text, (size_t)new_text_len * sizeof(ImWchar));
...
{
	const int new_text_len = new_text_end ? (int)(new_text_end - new_text) : (int)SpoofWinFuncs::strlenf_(new_text);
	if (new_text_len + BufTextLen + 1 >= BufSize)
		return;

	if (BufTextLen != pos)
		memmove(Buf + pos + new_text_len, Buf + pos, (size_t)(BufTextLen - pos));
	SpoofWinFuncs::memcpyf_(Buf + pos, new_text, (size_t)new_text_len * sizeof(char));
...
bool ImGui::InputTextEx(const char* label, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiTextEditCallback callback, void* user_data)
{
	...
	if (g.ActiveId == id && !g.ActiveIdIsJustActivated && !clear_active_id)
	{
		...
		else if (is_paste)
		{
			...
			//	const int clipboard_len = (int)SpoofWinFuncs::strlenf_(clipboard);
		...
		if (apply_edit_back_to_user_buffer)
		{
			...
			if ((flags & (ImGuiInputTextFlags_CallbackCompletion | ImGuiInputTextFlags_CallbackHistory | ImGuiInputTextFlags_CallbackAlways)) != 0)
			{
				...
				if (event_flag)
				{
					...
					{
						IM_ASSERT(callback_data.BufTextLen == (int)SpoofWinFuncs::strlenf_(callback_data.Buf)); // You need to maintain BufTextLen if you change the text!
						edit_state.CurLenW = ImTextStrFromUtf8(edit_state.Text.Data, edit_state.Text.Size, callback_data.Buf, NULL);
						edit_state.CurLenA = callback_data.BufTextLen;  // Assume correct length and valid UTF-8 from user, saves us an extra SpoofWinFuncs::strlenf_()
						edit_state.CursorAnimReset();
					}
				}
			}
	...
	const bool is_currently_scrolling = (edit_state.Id == id && is_multiline && g.ActiveId == draw_window->GetIDNoKeepAlive(skCrypt("#SCROLLY")));
...
bool ImGui::InputScalarEx(const char* label, ImGuiDataType data_type, void* data_ptr, void* step_ptr, void* step_fast_ptr, const char* scalar_format, ImGuiInputTextFlags extra_flags)
{
	...
	if (step_ptr)
	{
		PopItemWidth();
		SameLine(0, style.ItemInnerSpacing.x);
		if (ButtonEx(skCrypt("-"), button_sz, ImGuiButtonFlags_Repeat | ImGuiButtonFlags_DontClosePopups))
		{
...
bool ImGui::InputFloat(const char* label, float* v, float step, float step_fast, int decimal_precision, ImGuiInputTextFlags extra_flags)
{
	...
	if (decimal_precision < 0)
		SpoofWinFuncs::strcpyf_(display_format, skCrypt("%f"));      // Ideally we'd have a minimum decimal precision of 1 to visually denote that this is a float, while hiding non-significant digits? %f doesn't have a minimum of 1
	else
		ImFormatString(display_format, IM_ARRAYSIZE(display_format), skCrypt("%%.%df"), decimal_precision);
...
bool ImGui::InputInt(const char* label, int* v, int step, int step_fast, ImGuiInputTextFlags extra_flags)
{
	...
	const char* scalar_format = (extra_flags & ImGuiInputTextFlags_CharsHexadecimal) ? skCrypt("%08X") : skCrypt("%d");
...
bool ImGui::InputFloatN(const char* label, float* v, int components, int decimal_precision, ImGuiInputTextFlags extra_flags)
{
	...
	{
		PushID(i);
		value_changed |= InputFloat(skCrypt("##v"), &v[i], 0, 0, decimal_precision, extra_flags);
		SameLine(0, g.Style.ItemInnerSpacing.x);
		PopID();
		PopItemWidth();
	}
...
bool ImGui::InputIntN(const char* label, int* v, int components, ImGuiInputTextFlags extra_flags)
{
	...
	{
		PushID(i);
		value_changed |= InputInt(skCrypt("##v"), &v[i], 0, 0, extra_flags);
		SameLine(0, g.Style.ItemInnerSpacing.x);
		PopID();
		PopItemWidth();
	}
...
bool ImGui::BeginCombo(const char* label, const char* preview_value, ImGuiComboFlags flags)
{
	...
	char name[16];
	ImFormatString(name, IM_ARRAYSIZE(name), skCrypt("##Combo_%02d"), g.CurrentPopupStack.Size); // Recycle windows based on depth
...
bool ImGui::Combo(const char* label, int* current_item, bool(*items_getter)(void*, int, const char**), void* data, int items_count, int popup_max_height_in_items)
{
	...
	for (int i = 0; i < items_count; i++)
	{
		...
		if (!items_getter(data, i, &item_text))
			item_text = skCrypt("*Unknown item*");
...
bool ImGui::BeginMainMenuBar()
{
	...
	if (!Begin(skCrypt("##MainMenuBar"), NULL, ImGuiWindowFlags_NoTitleBar | ImGuiWindowFlags_NoResize | ImGuiWindowFlags_NoMove | ImGuiWindowFlags_NoScrollbar | ImGuiWindowFlags_NoSavedSettings | ImGuiWindowFlags_MenuBar)
		|| !BeginMenuBar())
	{
		End();
		PopStyleVar(2);
...
void ImGui::ColorTooltip(const char* text, const float* col, ImGuiColorEditFlags flags)
{
	...
	ColorButton(skCrypt("##preview"), ImVec4(col[0], col[1], col[2], col[3]), (flags & (ImGuiColorEditFlags_NoAlpha | ImGuiColorEditFlags_AlphaPreview | ImGuiColorEditFlags_AlphaPreviewHalf)) | ImGuiColorEditFlags_NoTooltip, sz);
	SameLine();
	if (flags & ImGuiColorEditFlags_NoAlpha)
		Text(skCrypt("#%02X%02X%02X\nR: %d, G: %d, B: %d\n(%.3f, %.3f, %.3f)"), cr, cg, cb, cr, cg, cb, col[0], col[1], col[2]);
	else
		Text(skCrypt("#%02X%02X%02X%02X\nR:%d, G:%d, B:%d, A:%d\n(%.3f, %.3f, %.3f, %.3f)"), cr, cg, cb, ca, cr, cg, cb, ca, col[0], col[1], col[2], col[3]);
	EndTooltip();
}
...
void ImGui::ColorEditOptionsPopup(const float* col, ImGuiColorEditFlags flags)
{
	...
	if ((!allow_opt_inputs && !allow_opt_datatype) || !BeginPopup(skCrypt("context")))
		return;
	ImGuiContext& g = *GImGui;
	ImGuiColorEditFlags opts = g.ColorEditOptions;
	if (allow_opt_inputs)
	{
		if (RadioButton(skCrypt("RGB"), (opts & ImGuiColorEditFlags_RGB) ? 1 : 0)) opts = (opts & ~ImGuiColorEditFlags__InputsMask) | ImGuiColorEditFlags_RGB;
		if (RadioButton(skCrypt("HSV"), (opts & ImGuiColorEditFlags_HSV) ? 1 : 0)) opts = (opts & ~ImGuiColorEditFlags__InputsMask) | ImGuiColorEditFlags_HSV;
		if (RadioButton(skCrypt("HEX"), (opts & ImGuiColorEditFlags_HEX) ? 1 : 0)) opts = (opts & ~ImGuiColorEditFlags__InputsMask) | ImGuiColorEditFlags_HEX;
	}
	if (allow_opt_datatype)
	{
		if (allow_opt_inputs) Separator();
		if (RadioButton(skCrypt("0..255"), (opts & ImGuiColorEditFlags_Uint8) ? 1 : 0)) opts = (opts & ~ImGuiColorEditFlags__DataTypeMask) | ImGuiColorEditFlags_Uint8;
		if (RadioButton(skCrypt("0.00..1.00"), (opts & ImGuiColorEditFlags_Float) ? 1 : 0)) opts = (opts & ~ImGuiColorEditFlags__DataTypeMask) | ImGuiColorEditFlags_Float;
	}

	if (allow_opt_inputs || allow_opt_datatype)
		Separator();
	if (Button(skCrypt("Copy as.."), ImVec2(-1, 0)))
		OpenPopup(skCrypt("Copy"));
	if (BeginPopup(skCrypt("Copy")))
	{
		int cr = IM_F32_TO_INT8_SAT(col[0]), cg = IM_F32_TO_INT8_SAT(col[1]), cb = IM_F32_TO_INT8_SAT(col[2]), ca = (flags & ImGuiColorEditFlags_NoAlpha) ? 255 : IM_F32_TO_INT8_SAT(col[3]);
		char buf[64];
		ImFormatString(buf, IM_ARRAYSIZE(buf), skCrypt("(%.3ff, %.3ff, %.3ff, %.3ff)"), col[0], col[1], col[2], (flags & ImGuiColorEditFlags_NoAlpha) ? 1.0f : col[3]);
		if (Selectable(buf))
			//SetClipboardText(buf);
		ImFormatString(buf, IM_ARRAYSIZE(buf), skCrypt("(%d,%d,%d,%d)"), cr, cg, cb, ca);
		if (Selectable(buf))
			//SetClipboardText(buf);
		if (flags & ImGuiColorEditFlags_NoAlpha)
			ImFormatString(buf, IM_ARRAYSIZE(buf), skCrypt("0x%02X%02X%02X"), cr, cg, cb);
		else
			ImFormatString(buf, IM_ARRAYSIZE(buf), skCrypt("0x%02X%02X%02X%02X"), cr, cg, cb, ca);
		if (Selectable(buf))
			//SetClipboardText(buf);
		EndPopup();
	}
...
static void ColorPickerOptionsPopup(ImGuiColorEditFlags flags, const float* ref_col)
{
	...
	if ((!allow_opt_picker && !allow_opt_alpha_bar) || !ImGui::BeginPopup(skCrypt("context")))
		return;
	ImGuiContext& g = *GImGui;
	if (allow_opt_picker)
	{
		...
		for (int picker_type = 0; picker_type < 2; picker_type++)
		{
			...
			if (ImGui::Selectable(skCrypt("##selectable"), false, 0, picker_size)) // By default, Selectable() is closing popup
				g.ColorEditOptions = (g.ColorEditOptions & ~ImGuiColorEditFlags__PickerMask) | (picker_flags & ImGuiColorEditFlags__PickerMask);
			ImGui::SetCursorScreenPos(backup_pos);
			ImVec4 dummy_ref_col;
			SpoofWinFuncs::memcpyf_(&dummy_ref_col.x, ref_col, sizeof(float) * (picker_flags & ImGuiColorEditFlags_NoAlpha ? 3 : 4));
			ImGui::ColorPicker4(skCrypt("##dummypicker"), &dummy_ref_col.x, picker_flags);
			ImGui::PopID();
		}
		ImGui::PopItemWidth();
	}
	if (allow_opt_alpha_bar)
	{
		...
		ImGui::CheckboxFlags(skCrypt("Alpha Bar"), (unsigned int*)&g.ColorEditOptions, ImGuiColorEditFlags_AlphaBar);
	}
	ImGui::EndPopup();
}*/
...
bool ImGui::ColorEdit4(const char* label, float col[4], ImGuiColorEditFlags flags)
{
	...
	if ((flags & (ImGuiColorEditFlags_RGB | ImGuiColorEditFlags_HSV)) != 0 && (flags & ImGuiColorEditFlags_NoInputs) == 0)
	{
		...
		const bool hide_prefix = (w_item_one <= CalcTextSize((flags & ImGuiColorEditFlags_Float) ? skCrypt("M:0.000") : skCrypt("M:000")).x);
		...
		for (int n = 0; n < components; n++)
		{
			...
			if (!(flags & ImGuiColorEditFlags_NoOptions))
				OpenPopupOnItemClick(skCrypt("context"));
		}
		PopItemWidth();
		PopItemWidth();
	}
	else if ((flags & ImGuiColorEditFlags_HEX) != 0 && (flags & ImGuiColorEditFlags_NoInputs) == 0)
	{
		...
		char buf[64];
		if (alpha)
			ImFormatString(buf, IM_ARRAYSIZE(buf), skCrypt("#%02X%02X%02X%02X"), ImClamp(i[0], 0, 255), ImClamp(i[1], 0, 255), ImClamp(i[2], 0, 255), ImClamp(i[3], 0, 255));
		else
			ImFormatString(buf, IM_ARRAYSIZE(buf), skCrypt("#%02X%02X%02X"), ImClamp(i[0], 0, 255), ImClamp(i[1], 0, 255), ImClamp(i[2], 0, 255));
		PushItemWidth(w_items_all);
		if (InputText(skCrypt("##Text"), buf, IM_ARRAYSIZE(buf), ImGuiInputTextFlags_CharsHexadecimal | ImGuiInputTextFlags_CharsUppercase))
		{
			value_changed = true;
			char* p = buf;
			...
			i[0] = i[1] = i[2] = i[3] = 0;
			if (alpha)
				sscanf(p, skCrypt("%02X%02X%02X%02X"), (unsigned int*)&i[0], (unsigned int*)&i[1], (unsigned int*)&i[2], (unsigned int*)&i[3]); // Treat at unsigned (%X is unsigned)
			else
				sscanf(p, skCrypt("%02X%02X%02X"), (unsigned int*)&i[0], (unsigned int*)&i[1], (unsigned int*)&i[2]);
		}
		if (!(flags & ImGuiColorEditFlags_NoOptions))
			OpenPopupOnItemClick(skCrypt("context"));
		PopItemWidth();
	...
	if (!(flags & ImGuiColorEditFlags_NoSmallPreview))
	{
		...
		if (ColorButton(skCrypt("##ColorButton"), col_v4, flags))
		{
			if (!(flags & ImGuiColorEditFlags_NoPicker))
			{
				// Store current color and open a picker
				g.ColorPickerRef = col_v4;
				OpenPopup(skCrypt("picker"));
		...
		if (BeginPopup(skCrypt("picker")))
		{
			...
			value_changed |= ColorPicker4(skCrypt("##picker"), col, picker_flags, &g.ColorPickerRef.x);
			PopItemWidth();
			EndPopup();
		}
	}
	...
	if (window->DC.LastItemRectHoveredRect && BeginDragDropTarget()) // NB: The LastItemRectHoveredRect test is merely an optional micro-optimization
	{
		...
		{
			SpoofWinFuncs::memcpyf_((float*)col, payload->Data, sizeof(float) * 3);
			value_changed = true;
		}
		if (const ImGuiPayload* payload = AcceptDragDropPayload(IMGUI_PAYLOAD_TYPE_COLOR_4F))
		{
			SpoofWinFuncs::memcpyf_((float*)col, payload->Data, sizeof(float) * components);
			value_changed = true;
		}
		EndDragDropTarget();
	}
...
bool ImGui::ColorPicker4(const char* label, float col[4], ImGuiColorEditFlags flags, const float* ref_col)
{
	...
	SpoofWinFuncs::memcpyf_(backup_initial_col, col, components * sizeof(float));
	...
	if (flags & ImGuiColorEditFlags_PickerHueWheel)
	{
		...
		InvisibleButton(skCrypt("hsv"), ImVec2(sv_picker_size + style.ItemInnerSpacing.x + bars_width, sv_picker_size));
		if (IsItemActive())
		{
	...
	{
		// SV rectangle logic
		InvisibleButton(skCrypt("sv"), ImVec2(sv_picker_size, sv_picker_size));
		if (IsItemActive())
		{
		...
		InvisibleButton(skCrypt("hue"), ImVec2(bars_width, sv_picker_size));
		if (IsItemActive())
		{
	...
	if (alpha_bar)
	{
		...
		InvisibleButton(skCrypt("alpha"), ImVec2(bars_width, sv_picker_size));
		if (IsItemActive())
		{
	...
	if (!(flags & ImGuiColorEditFlags_NoSidePreview))
	{
		...
		if ((flags & ImGuiColorEditFlags_NoLabel))
			Text(skCrypt("Current"));
		ColorButton(skCrypt("##current"), col_v4, (flags & (ImGuiColorEditFlags_HDR | ImGuiColorEditFlags_AlphaPreview | ImGuiColorEditFlags_AlphaPreviewHalf | ImGuiColorEditFlags_NoTooltip)), ImVec2(square_sz * 3, square_sz * 2));
		if (ref_col != NULL)
		{
			Text(skCrypt("Original"));
			ImVec4 ref_col_v4(ref_col[0], ref_col[1], ref_col[2], (flags & ImGuiColorEditFlags_NoAlpha) ? 1.0f : ref_col[3]);
			if (ColorButton(skCrypt("##original"), ref_col_v4, (flags & (ImGuiColorEditFlags_HDR | ImGuiColorEditFlags_AlphaPreview | ImGuiColorEditFlags_AlphaPreviewHalf | ImGuiColorEditFlags_NoTooltip)), ImVec2(square_sz * 3, square_sz * 2)))
			{
				SpoofWinFuncs::memcpyf_(col, ref_col, components * sizeof(float));
				value_changed = true;
			}
		}
		EndGroup();
	...
	if ((flags & ImGuiColorEditFlags_NoInputs) == 0)
	{
		...
		if (flags & ImGuiColorEditFlags_RGB || (flags & ImGuiColorEditFlags__InputsMask) == 0)
			value_changed |= ColorEdit4(skCrypt("##rgb"), col, sub_flags | ImGuiColorEditFlags_RGB);
		if (flags & ImGuiColorEditFlags_HSV || (flags & ImGuiColorEditFlags__InputsMask) == 0)
			value_changed |= ColorEdit4(skCrypt("##hsv"), col, sub_flags | ImGuiColorEditFlags_HSV);
		if (flags & ImGuiColorEditFlags_HEX || (flags & ImGuiColorEditFlags__InputsMask) == 0)
			value_changed |= ColorEdit4(skCrypt("##hex"), col, sub_flags | ImGuiColorEditFlags_HEX);
		PopItemWidth();
	}
...
void ImGui::BeginColumns(const char* str_id, int columns_count, ImGuiColumnsFlags flags)
{
	...
	ImGuiID id = window->GetID(str_id ? str_id : skCrypt("columns"));
	PopID();
...
void ImGui::TreePush(const char* str_id)
{
	...
	Indent();
	window->DC.TreeDepth++;
	PushID(str_id ? str_id : skCrypt("#TreePush"));
}
...
void ImGui::TreePush(const void* ptr_id)
{
	...
	Indent();
	window->DC.TreeDepth++;
	PushID(ptr_id ? ptr_id : (const void*)skCrypt("#TreePush"));
}
...
{
	Text(skCrypt("%s: %s"), prefix, (b ? skCrypt("true") : skCrypt("false")));
}

void ImGui::Value(const char* prefix, int v)
{
	Text(skCrypt("%s: %d"), prefix, v);
}
...
void ImGui::Value(const char* prefix, float v, const char* float_format)
{
	...
	{
		char fmt[64];
		ImFormatString(fmt, IM_ARRAYSIZE(fmt), skCrypt("%%s: %s"), float_format);
		Text(fmt, prefix, v);
	}
	else
	{
		Text(skCrypt("%s: %.3f"), prefix, v);
	}
}
...
bool ImGui::BeginDragDropSource(ImGuiDragDropFlags flags, int mouse_button)
{
	...
	{
		window = NULL;
		source_id = ImHash(skCrypt("#SourceExtern"), 0);
...
bool ImGui::SetDragDropPayload(const char* type, const void* data, size_t data_size, ImGuiCond cond)
{
	...
	IM_ASSERT(type != NULL);
	IM_ASSERT(SpoofWinFuncs::strlenf_(type) < IM_ARRAYSIZE(payload.DataType) && skCrypt("Payload type can be at most 12 characters long"));
	...
	if (cond == ImGuiCond_Always || payload.DataFrameCount == -1)
	{
		...
		if (data_size > sizeof(g.DragDropPayloadBufLocal))
		{
			...
			SpoofWinFuncs::memcpyf_((void*)payload.Data, data, data_size);
		}
		else if (data_size > 0)
		{
			// Store locally
...
//{
//	if (ImGui::Begin(skCrypt("ImGui Metrics"), p_open))
//	{
//		ImGui::Text(skCrypt("Dear ImGui %s"), ImGui::GetVersion());
//		ImGui::Text(skCrypt("Application average %.3f ms/frame (%.1f FPS)"), 1000.0f / ImGui::GetIO().Framerate, ImGui::GetIO().Framerate);
//		ImGui::Text(skCrypt("%d vertices, %d indices (%d triangles)"), ImGui::GetIO().MetricsRenderVertices, ImGui::GetIO().MetricsRenderIndices, ImGui::GetIO().MetricsRenderIndices / 3);
//		ImGui::Text(skCrypt("%d allocations"), ImGui::GetIO().MetricsAllocs);
//		static bool show_clip_rects = true;
//		ImGui::Checkbox(skCrypt("Show clipping rectangles when hovering an ImDrawCmd"), &show_clip_rects);
//		ImGui::Separator();
//
//		struct Funcs
//		{
...
//			{
//				bool node_open = ImGui::TreeNode(draw_list, skCrypt("%s: '%s' %d vtx, %d indices, %d cmds"), label, draw_list->_OwnerName ? draw_list->_OwnerName : "", draw_list->VtxBuffer.Size, draw_list->IdxBuffer.Size, draw_list->CmdBuffer.Size);
//				if (draw_list == ImGui::GetWindowDrawList())
//				{
//					ImGui::SameLine();
//					ImGui::TextColored(ImColor(255, 100, 100), skCrypt("CURRENTLY APPENDING")); // Can't display stats for active draw list! (we don't have the data double-buffered)
...
//					{
//						ImGui::BulletText(skCrypt("Callback %p, user_data %p"), pcmd->UserCallback, pcmd->UserCallbackData);
//						continue;
//					}
//					ImDrawIdx* idx_buffer = (draw_list->IdxBuffer.Size > 0) ? draw_list->IdxBuffer.Data : NULL;
//					bool pcmd_node_open = ImGui::TreeNode((void*)(pcmd - draw_list->CmdBuffer.begin()), skCrypt("Draw %-4d %s vtx, tex = %p, clip_rect = (%.0f,%.0f)..(%.0f,%.0f)"), pcmd->ElemCount, draw_list->IdxBuffer.Size > 0 ? "indexed" : "non-indexed", pcmd->TextureId, pcmd->ClipRect.x, pcmd->ClipRect.y, pcmd->ClipRect.z, pcmd->ClipRect.w);
...
//								buf_p += ImFormatString(buf_p, (int)(buf_end - buf_p), skCrypt("%s %04d { pos = (%8.2f,%8.2f), uv = (%.6f,%.6f), col = %08X }\n"), (n == 0) ? skCrypt("vtx") : skCrypt("   "), vtx_i, v.pos.x, v.pos.y, v.uv.x, v.uv.y, v.col);
//							}
...
//			{
//				if (!ImGui::TreeNode(label, skCrypt("%s (%d)"), label, windows.Size))
//					return;
//				for (int i = 0; i < windows.Size; i++)
//					Funcs::NodeWindow(windows[i], skCrypt("Window"));
//				ImGui::TreePop();
//			}
//
//			static void NodeWindow(ImGuiWindow* window, const char* label)
//			{
//				if (!ImGui::TreeNode(window, skCrypt("%s '%s', %d @ 0x%p"), label, window->Name, window->Active || window->WasActive, window))
//					return;
//				NodeDrawList(window, window->DrawList, skCrypt("DrawList"));
//				ImGui::BulletText(skCrypt("Pos: (%.1f,%.1f), Size: (%.1f,%.1f), SizeContents (%.1f,%.1f)"), window->Pos.x, window->Pos.y, window->Size.x, window->Size.y, window->SizeContents.x, window->SizeContents.y);
//				ImGui::BulletText(skCrypt("Scroll: (%.2f/%.2f,%.2f/%.2f)"), window->Scroll.x, GetScrollMaxX(window), window->Scroll.y, GetScrollMaxY(window));
//				ImGui::BulletText(skCrypt("Active: %d, WriteAccessed: %d"), window->Active, window->WriteAccessed);
//				if (window->RootWindow != window) NodeWindow(window->RootWindow, skCrypt("RootWindow"));
//				if (window->DC.ChildWindows.Size > 0) NodeWindows(window->DC.ChildWindows, skCrypt("ChildWindows"));
//				ImGui::BulletText(skCrypt("Storage: %d bytes"), window->StateStorage.Data.Size * (int)sizeof(ImGuiStorage::Pair));
//				ImGui::TreePop();
//			}
//		};
//
...
//		ImGuiContext& g = *GImGui;
//		Funcs::NodeWindows(g.Windows, skCrypt("Windows"));
//		if (ImGui::TreeNode(skCrypt("DrawList"), skCrypt("Active DrawLists (%d)"), g.DrawDataBuilder.Layers[0].Size))
//		{
//			for (int i = 0; i < g.DrawDataBuilder.Layers[0].Size; i++)
//				Funcs::NodeDrawList(NULL, g.DrawDataBuilder.Layers[0][i], skCrypt("DrawList"));
//			ImGui::TreePop();
//		}
//		if (ImGui::TreeNode(skCrypt("Popups"), skCrypt("Open Popups Stack (%d)"), g.OpenPopupStack.Size))
//		{
//			for (int i = 0; i < g.OpenPopupStack.Size; i++)
//			{
//				ImGuiWindow* window = g.OpenPopupStack[i].Window;
//				ImGui::BulletText(skCrypt("PopupID: %08x, Window: '%s'%s%s"), g.OpenPopupStack[i].PopupId, window ? window->Name : xorstrstr("NULL"), window && (window->Flags & ImGuiWindowFlags_ChildWindow) ? skCrypt(" ChildWindow") : skCrypt(""), window && (window->Flags & ImGuiWindowFlags_ChildMenu) ? skCrypt(" ChildMenu") : skCrypt(""));
//			}
//			ImGui::TreePop();
//		}
//		if (ImGui::TreeNode(skCrypt("Basic state")))
//		{
//			ImGui::Text(skCrypt("HoveredWindow: '%s'"), g.HoveredWindow ? g.HoveredWindow->Name : xorstrstr("NULL"));
//			ImGui::Text(skCrypt("HoveredRootWindow: '%s'"), g.HoveredRootWindow ? g.HoveredRootWindow->Name : xorstrstr("NULL"));
//			ImGui::Text(skCrypt("HoveredId: 0x%08X/0x%08X (%.2f sec)"), g.HoveredId, g.HoveredIdPreviousFrame, g.HoveredIdTimer); // Data is "in-flight" so depending on when the Metrics window is called we may see current frame information or not
//			ImGui::Text(skCrypt("ActiveId: 0x%08X/0x%08X (%.2f sec)"), g.ActiveId, g.ActiveIdPreviousFrame, g.ActiveIdTimer);
//			ImGui::Text(skCrypt("ActiveIdWindow: '%s'"), g.ActiveIdWindow ? g.ActiveIdWindow->Name : xorstrstr("NULL"));
//			ImGui::Text(skCrypt("NavWindow: '%s'"), g.NavWindow ? g.NavWindow->Name : xorstrstr("NULL"));
//			ImGui::Text(skCrypt("DragDrop: %d, SourceId = 0x%08X, Payload \"%s\" (%d bytes)"), g.DragDropActive, g.DragDropPayload.SourceId, g.DragDropPayload.DataType, g.DragDropPayload.DataSize);
//			ImGui::TreePop();
//		}
//	}
//	ImGui::End();
